...
int maxSubsetSum(vector<int>& b) {
    ...
    for (int i = 2; i <= n; ++i) {
        // dp[i][2] = b[i] + b[i-1] + max(dp[i-3][2], max(p2[i-4], p3[i-4]))
        int opt1 = b[i] + b[i-1] + ((i - 3 >= 0) ? dp[i - 3][2] : 0);
        int opt2 = b[i] + b[i-1] + ((i - 4 >= 0) ? max(p2[i - 4], p3[i - 4]) : 0);
        dp[i][2] = max(opt1, opt2);

        if (i >= 3) {
            // dp[i][3] = b[i] + b[i-1] + b[i-2] + max(p2[i-5], p3[i-5])
            int prevMax = (i - 5 >= 0) ? max(p2[i - 5], p3[i - 5]) : 0;
            dp[i][3] = b[i] + b[i - 1] + b[i - 2] + prevMax;
        }

        p2[i] = max(p2[i - 1], dp[i][2]);
        p3[i] = max(p3[i - 1], dp[i][3]);24
