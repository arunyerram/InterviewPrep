...
def max_happiness(n, a, b, c):
    ...
    for i in range(2, n):
        dpa[i] = max(
            a[i] + max(dpb[i - 1], dpc[i - 1]),
            a[i] + a[i - 1] + max(dpb[i - 2], dpc[i - 2])
        )
        dpb[i] = max(
            b[i] + max(dpa[i - 1], dpc[i - 1]),
            b[i] + b[i - 1] + max(dpa[i - 2], dpc[i - 2])
        )
        dpc[i] = max(
            c[i] + max(dpa[i - 1], dpb[i - 1]),
            c[i] + c[i - 1] + max(dpa[i - 2], dpb[i - 2])
        )
