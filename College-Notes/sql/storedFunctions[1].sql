...
CREATE function getInfo(p_empNo int) returns varchar(80)
    DETERMINISTIC
    BEGIN
    ...
    SELECT ename INTO empName,job INTO empJob, sal INTO empSale, mname INTO empManager
    FROM emp
    WHERE empno = p_empNo;

    return concat(empName, ' ', empJob, ' ', empSal, ' ', empManager);
