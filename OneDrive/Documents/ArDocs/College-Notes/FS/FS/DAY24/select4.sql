...
Select deptno, sum(sal), count(*) from emp where deptno <> 10 Group By deptno;

Select deptno, job, sum(sal), count(sal) from emp Group By deptno, job;
...
Select deptno, sum(sal) from emp Group By deptno Having sum(sal) > 1000 Order By sum(sal) DESC;

Select deptno, sum(sal) from emp where ename <> 'Kevin' Group By deptno;

Select deptno, sum(sal) from emp where ename <> 'Kevin' Group By deptno Having sum(sal) > 6000;