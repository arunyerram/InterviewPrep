...
select getDeptSal(20);
    ...
    -- select concat(empno, ename, job, mgr, hiredate, sal, comm, deptno) INTO empDetails from emp where empno = empno;

DELIMITER $$
CREATE function getEmpDetails(eno int) returns varchar(1000)
BEGIN
    DECLARE empDetails varchar(1000);
    SET empDetails = (select concat(empno, " ",ename, " ",job, " ",mgr) from emp where empno = eno);
  return empDetails; 
END$$
