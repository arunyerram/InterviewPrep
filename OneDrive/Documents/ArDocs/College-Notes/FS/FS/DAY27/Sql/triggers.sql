...
BEGIN
    IF (OLD.age < 20) THEN
        SELECT 'Warning: Old age was less than 20';
    END IF;
    IF (NEW.age < 20) THEN
        SIGNAL SQLSTATE '02000' 
        SET MESSAGE_TEXT = 'Warning: Age cannot be less than 20 (cannot update)';
    END IF;
END $$
...
BEGIN
    IF (OLD.age < 20) THEN
        ...
        SET MESSAGE_TEXT = 'Warning: Old age was less than 20';
    END IF;
    IF (NEW.age < 20) THEN
        SIGNAL SQLSTATE '02000' 
        SET MESSAGE_TEXT = 'Warning: Age cannot be less than 20 (cannot update)';
